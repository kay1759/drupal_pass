defmodule WhiteBreadContext do
  ...
  when_(
    ~r/^I input plain password "(?<password>[^"]+)"$/,
  ...
  and_(
    ~r/^encrypted password is "(?<encrypted>[^"]+)"$/,
  ...
  then_(
    ~r/^I should see "(?<result>[^"]+)"$/,
    fn state, %{result: result} ->
      password = state[:password]
      encrypted = state[:encrypted]
      assert DrupalPass.check_password(password, encrypted) === (result == "true")
